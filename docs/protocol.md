...
Native connections use directly driven Quinn protocol state and the upstream MoQ machines pinned in Cargo.toml. Socket-address dialing preserves scoped IPv6. TLS policy lives in `transport/src/tls.rs`. The native client uses raw QUIC. The same leader listener also accepts WebTransport CONNECT at `https://IP:PORT/`, without a relay or separate HTTP server. Browser clients offer only `moq-lite-05` in WebTransport `protocols` and check the negotiated MoQ version. All application messages, tracks, datagrams, and monotonic timestamp units below are identical across both transports; no browser-specific wire version is introduced.
