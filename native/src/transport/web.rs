// Adapted from moq-dev/moq moq-uring at 5d0991b9991305be907e6c0682a4e276722eeed0.
// Original licensed MIT OR Apache-2.0; see native/licenses.
    ...
    fn drop(&mut self) {
        if let Some(conn) = self.conn.take() {
            conn.close_code(
                ...
                "webtransport handshake abandoned",
            );
        }
    }
}
...
impl Request {
    ...
    pub async fn accept(handle: &Driver, conn: Connection) -> Result<Self, Error> {
    ...
    async fn handshake(handle: &Driver, mut conn: Connection) -> Result<Self, Error> {
    ...
    /// [re-exported](crate::transport::url) so naming this type needs no dependency of
    /// your own.
    ...
    pub async fn respond(mut self, response: Response) -> Result<Session, Error> {
        ...
        if let Some(protocol) = &protocol {
                ...
                .any(|offered| offered == protocol)
            {
                return Err(Error::Web(format!(
                    "subprotocol {protocol:?} was not offered"
                )));
            }
        ...
        encoded
            .encode(&mut buf)
            .map_err(|err| Error::Web(err.to_string()))?;
...
struct Web {
    recv: RefCell<super::RecvStream>,
    capsules: RefCell<Capsules>,
...
impl Session {
    ...
    fn establish(request: Request, protocol: Option<String>) -> Self {
        ...
        let web = Rc::new(Web {
            ...
            capsules: RefCell::new(Capsules::default()),
...
impl web_transport_trait::poll::Session for Session {
    ...
    fn poll_accept_uni(
        ...
        let Some(web) = self.web.clone() else {
            let inner = ready!(web_transport_trait::poll::Session::poll_accept_uni(
                &mut self.conn,
                cx
            ))?;
        ...
        loop {
            ...
            loop {
                match web_transport_trait::poll::Session::poll_accept_uni(&mut self.conn, cx) {
                    Poll::Ready(Ok(recv)) => web
                        .state
                        .borrow_mut()
                        .pending_uni
                        .push(PendingUni::new(recv)),
    ...
    fn poll_accept_bi(
        ...
        let Some(web) = self.web.clone() else {
            let (send, recv) = ready!(web_transport_trait::poll::Session::poll_accept_bi(
                &mut self.conn,
                cx
            ))?;
            return Poll::Ready(Ok((
        ...
        loop {
            ...
            loop {
                match web_transport_trait::poll::Session::poll_accept_bi(&mut self.conn, cx) {
                    Poll::Ready(Ok((send, recv))) => web
                        .state
                        .borrow_mut()
                        .pending_bi
                        .push(PendingBi::new(send, recv)),
                    Poll::Ready(Err(err)) => return Poll::Ready(Err(unmap_err(err))),
                    Poll::Pending => break,
                }
            }
    ...
    fn poll_open_uni(
        ...
        let inner = ready!(web_transport_trait::poll::Session::poll_open_uni(
            &mut self.conn,
            cx
        ))
    ...
    fn poll_open_bi(
        ...
        let (send, recv) = ready!(web_transport_trait::poll::Session::poll_open_bi(
            &mut self.conn,
            cx
        ))
        .map_err(|err| self.map_err(err))?;
    ...
    fn poll_send_datagram(
        ...
        let Some(web) = &self.web else {
            return web_transport_trait::poll::Session::poll_send_datagram(
                &mut self.conn,
                cx,
                payload,
            );
    ...
    fn close(&mut self, code: u32, reason: &str) {
        ...
        let Some(send) = connect_send else {
        ...
        *web.closing.borrow_mut() = Some(Closing {
            send,
            frame,
            offset: 0,
    ...
    fn poll_closed(&mut self, cx: &mut Context<'_>) -> Poll<Self::Error> {
        let err = ready!(web_transport_trait::poll::Session::poll_closed(
            &mut self.conn,
            cx
        ));
        let Some(web) = &self.web else {
...
fn unmap_err(err: Error) -> Error {
    ...
    match err {
        ...
        Error::Closed { code, reason } => match unmap(code) {
            Some(code) => Error::Closed { code, reason },
        ...
        match stream.poll_classify(cx) {
            ...
            Poll::Ready(Some(session)) if session == web.session_id => {
                web.state
                    .borrow_mut()
                    .ready_bi
                    .push_back((stream.send, stream.recv));
                progressed = true;
            }
            Poll::Ready(_) => {}
        }
...
async fn write_all(send: &mut super::SendStream, mut buf: &[u8]) -> Result<(), Error> {
    while !buf.is_empty() {
        let n = std::future::poll_fn(|cx| {
            web_transport_trait::poll::SendStream::poll_write(send, cx, buf)
        })
        .await?;
        buf = &buf[n..];
    }
...
async fn read_some(recv: &mut super::RecvStream, buf: &mut BytesMut) -> Result<bool, Error> {
    ...
    let n = std::future::poll_fn(|cx| {
        web_transport_trait::poll::RecvStream::poll_read(recv, cx, &mut chunk)
    })
    .await?;
    match n {
        Some(n) => {
            if buf.len() + n > MESSAGE_LIMIT {
                return Err(Error::Web(
                    "an HTTP/3 message exceeded the buffer limit".into(),
                ));
            }
...
/// HTTP/3 control work owned and stepped by the networking worker.
/// This registry runs no tasks and starts no thread.
type Sessions = Rc<RefCell<Vec<(Connection, Rc<Web>)>>>;
...
impl Driver {
    ...
    pub fn next_deadline(&self) -> Option<Instant> {
        self.sessions
            .borrow()
            .iter()
            .filter_map(|(_, w)| w.closing.borrow().as_ref().map(|c| c.deadline))
            .min()
    }
    pub fn step(&self, cx: &mut Context<'_>, now: Instant) {
        use web_transport_trait::poll::{RecvStream as _, SendStream as _, Session as _};
        self.sessions.borrow_mut().retain_mut(|(conn, web)| {
            if conn.poll_closed(cx).is_ready() {
                return false;
            }
            if let Some(close) = web.closing.borrow_mut().as_mut() {
                if now >= close.deadline {
                    conn.close_code(close.code, &close.reason);
                    return false;
                }
                for _ in 0..8 {
                    if close.offset == close.frame.len() {
                        break;
                    }
                    match close.send.poll_write(cx, &close.frame[close.offset..]) {
            ...
            let mut capsules = web.capsules.borrow_mut();
            for operation in 0..8 {
                ...
                match capsules.take() {
                    Ok(Some(proto::Capsule::CloseWebTransportSession { code, reason })) => {
                        web.closed.borrow_mut().get_or_insert(Error::Closed {
                            code: code.into(),
                            reason: reason.clone(),
                        });
                    ...
                    Ok(Some(_)) => continue,
                    Err(_) => {
                        conn.close_code(H3_GENERAL_PROTOCOL_ERROR, "invalid capsule");
                        return false;
                    }
                    Ok(None) => {}
                }
                let mut buffer = [0; 4096];
                match web.recv.borrow_mut().poll_read(cx, &mut buffer) {
                    Poll::Ready(Ok(Some(n))) => {
                        if capsules.frames.len() + n > MESSAGE_LIMIT {
                            conn.close_code(H3_GENERAL_PROTOCOL_ERROR, "capsule too large");
                            return false;
                        }
                        capsules.frames.extend_from_slice(&buffer[..n]);
                    }
                    Poll::Pending => break,
                    _ => {
                        conn.close_code(proto::error_to_http3(0), "");
                        return false;
                    }
                }
            }
...
impl Capsules {
    ...
    fn demux(&mut self) -> Result<(), Error> {
        loop {
            ...
            let len =
                usize::try_from(len).map_err(|_| Error::Web("oversized HTTP/3 frame".into()))?;
            if peek.len() < len {
                return Ok(());
            }
...
mod tests {
    ...
    fn an_unmappable_code_is_not_an_application_error() {
        ...
        assert_eq!(
            h3.session_error(),
            None,
            "not a code the peer's application chose"
        );

        let reset = unmap_err(Error::Reset(0x100));
        assert!(
            matches!(reset, Error::Http3 { code: 0x100, .. }),
            "got {reset:?}"
        );
    ...
    fn a_capsule_spans_data_frames() {
        ...
        assert!(
            capsules.take().expect("parse").is_none(),
            "half a capsule is not one"
        );
    ...
    fn one_frame_carries_several_capsules() {
        ...
        capsules
            .frames
            .extend_from_slice(&frame(FRAME_DATA, &payload));
    ...
    fn a_capsule_stream_is_bounded() {
        ...
        capsules
            .frames
            .extend_from_slice(&frame(FRAME_DATA, &header));
        ...
        let err = loop {
            ...
            capsules
                .frames
                .extend_from_slice(&frame(FRAME_DATA, &chunk));
        };
        assert!(matches!(err, Error::Web(_)), "refused with {err}");
        assert!(
            capsules.body.len() <= MESSAGE_LIMIT,
            "the buffer stayed bounded"
        );
    }
}
