...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="tidkod-apple-") as temporary:
        work = Path(temporary)
        env = dict(os.environ, TIDKOD_SDK_OUT=str(work / "package"), TIDKOD_BUILD_PROFILE="release")
        run("bash", "scripts/build-apple-sdk.sh", env=env)
        sdk = work / "package/Tidkod"
        ...
        archive_path = archive(sdk, ROOT / "dist/releases", f"tidkod-{version()}-apple-xcframework", windows=True)
        extracted = unpack(archive_path, work / "extracted")
        info = plistlib.loads((extracted / "RustTidkod.xcframework/Info.plist").read_bytes())
        assert {i["LibraryIdentifier"] for i in info["AvailableLibraries"]} == {
            "macos-arm64_x86_64", "ios-arm64", "ios-arm64-simulator"}
        architectures = run("xcrun", "lipo", "-archs", extracted / "RustTidkod.xcframework/macos-arm64_x86_64/libtidkod_bindings.a", capture=True)
        ...
        try:
            ...
            run("xcodebuild", "test", "-project", project / "TidkodSmoke.xcodeproj",
                "-scheme", "TidkodSmoke", "-configuration", "Release",
