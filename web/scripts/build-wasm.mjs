...
run('cargo', ['build', '--locked', '-p', 'ethersync-wasm', '--target', 'wasm32-unknown-unknown', '--release']);
for (const [target, dir] of [['web', 'pkg'], ['nodejs', 'pkg-node']]) {
  run(bindgen, [resolve(root, 'target/wasm32-unknown-unknown/release/ethersync_wasm.wasm'), '--target', target, '--out-dir', resolve(web, dir)]);
