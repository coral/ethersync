...
use ethersync_transport::{Endpoint, PendingConnection, moq, runtime::Runtime, web};
use std::{
    future::Future,
    pin::Pin,
    sync::{
...
struct Peer {
    connection: Option<ethersync_transport::Connection>,
...
impl LeaderJob {
    ...
    pub fn new(
        ...
        let create = || -> Result<(Io, LeaderInfo)> {
            let (server, fingerprint) = ethersync_transport::tls::server().map_err(transport)?;
