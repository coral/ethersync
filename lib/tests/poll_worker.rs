use ethersync::{Engine, FollowerConfig, LeaderConfig, Rate, SyncState};
...
fn idle_worker_sleeps_and_command_wakeup_publishes_before_ack() {
    ...
    leader.seek(ethersync::Position::from_frames(42)).unwrap();
    assert_eq!(reader.read().position, ethersync::Position::from_frames(42));
    engine.shutdown().unwrap();
}
