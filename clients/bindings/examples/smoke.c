...
int main(void) {
    ...
    CHECK(ethersync_core_snapshot(core, &snapshot, &error) == 0);
    CHECK(ethersync_timecode_snapshot_copy(snapshot, &copy, &error) == 0);
    CHECK(ethersync_core_snapshot_into(core, snapshot, &error) == 0);
    CHECK(ethersync_timecode_snapshot_read(snapshot, 123, &value, &error) == 0);
    CHECK(value.accepted_observations == 0);
    Boundary boundary;
    CHECK(ethersync_timecode_snapshot_next_boundary(snapshot, 123, &boundary, &error) == 0);
    CHECK(!boundary.valid);
    CHECK(ethersync_timecode_snapshot_read_for_presentation(snapshot, 123, 1, &value, &error) == 0);
    ...
    CHECK(ethersync_timecode_snapshot_read(copy, 123, &value, &error) == 0);
    ethersync_timecodesnapshot_free(copy);
    ...
    CHECK(ethersync_reader_snapshot_into(reader, snapshot, &error) == 0);
    EndpointList *endpoints = NULL;
    Endpoint *endpoint = NULL;
    uint32_t count = 0;
    CHECK(ethersync_leader_local_endpoints(leader, &endpoints, &error) == 0);
    CHECK(ethersync_endpoint_list_count(endpoints, &count, &error) == 0 && count == 1);
    CHECK(ethersync_endpoint_list_get(endpoints, 0, &endpoint, &error) == 0);
    ethersync_endpoint_free(endpoint);
    CHECK(ethersync_endpoint_list_get(endpoints, count, &endpoint, &error) != 0);
    ethersync_buffer_free(error); error = NULL;
    ethersync_endpointlist_free(endpoints);
...
#endif
    ethersync_timecodesnapshot_free(snapshot);
