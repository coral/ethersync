use ethersync_transport::{Endpoint, moq, runtime::Runtime, tls};
use std::{
    future::Future,
    pin::pin,
...
fn moq_lite_handshake_and_track_without_executor() {
    ...
    let mut client_handshake = pin!(
        client_builder.connect_lite(runtime.clone(), ethersync_transport::web::Session::raw(a))
    );
    let mut server_handshake = pin!(
        server_builder.accept_lite(runtime.clone(), ethersync_transport::web::Session::raw(b))
    );
